...
import SlideProductInnovationClosing from "./SlideProductInnovationClosing";
...
import CompetitiveAdvantageSlide from "./CompetitiveAdvantageSlide";
import MarketPotentialSlide from "./MarketPotentialSlide";
import ImpactSlide from "./ImpactSlide";
import TeamExperienceSlide from "./TeamExperienceSlide";
...
function OperatorGallery({ slide, reduceMotion }: { slide: PitchSlide; reduceMotion: boolean }) {
  return (
    <div className="operator-gallery" aria-label="Trayectoria real como tour operador">
      ...
      <motion.div className="operator-gallery__panel" initial={reduceMotion ? false : { opacity: 0, x: -42, filter: "blur(8px)" }} animate={{ opacity: 1, x: 0, filter: "blur(0px)" }} transition={{ delay: reduceMotion ? 0 : .48, duration: .82, ease: [0.16, 1, 0.3, 1] }}>
        <span className="operator-gallery__eyebrow"><i />{slide.eyebrow}</span>
        <h1>{slide.title}</h1>
        <p className="operator-gallery__statement">{slide.statement}</p>
        <div className="operator-gallery__signals">
          {slide.bullets?.map((item, itemIndex) => (
            <motion.span key={item} initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 1.05 + itemIndex * .14, duration: .45 }}>
              <b>{String(itemIndex + 1).padStart(2, "0")}</b>{item}
            </motion.span>
          ))}
        </div>
      </motion.div>
      <motion.div className="operator-gallery__proof" initial={reduceMotion ? false : { opacity: 0, scale: .7 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: reduceMotion ? 0 : 1.35, type: "spring", stiffness: 150, damping: 18 }}><strong>EXPERIENCIA</strong><span>DE PRIMERA MANO</span></motion.div>
    </div>
  );
}
...
export default function PitchDeck() {
  ...
  const immersive = current.kind === "cover" || current.kind === "innovation" || current.kind === "innovationClosing" || current.kind === "product" || current.kind === "gallery" || current.kind === "question" || current.kind === "problemPoints" || current.kind === "marketPotential" || current.kind === "formation" || current.kind === "homefeedPanorama" || current.kind === "homefeedConnection" || current.kind === "eventModes" || current.kind === "transportIntegration" || current.kind === "touristArrival" || current.kind === "ecosystemImpact" || current.kind === "regionalPotential" || current.kind === "competitiveAdvantage" || current.kind === "impact" || current.kind === "businessModel" || current.kind === "founder" || current.kind === "closing";
  ...
  useEffect(() => {
    ...
    if (index < slides.length - 1) next();
      ...
      setAutoDemo(false);
      setRunning(false);
      setElapsed(totalPitchSeconds);
      setSlideElapsed(current.duration);
    }
  ...
  return (
    <main className={`deck-shell ${immersive ? "deck-shell--cover" : ""}`}>
      ...
      <header className="deck-header"><Brand compact /><div className="deck-header__meta"><span>{index === 0 ? "Apertura" : index === 1 ? "Necesidad del Mercado" : index === 2 ? "Innovación" : index === 3 ? "Propuesta de Valor" : index === 4 ? "Potencial de Mercado" : index === 5 ? "Escalabilidad" : index === 6 ? "Ventaja Competitiva" : index === 7 ? "Impacto" : index === 8 ? "Ingresos" : index === 9 ? "Track Record" : index === 10 ? "Experiencia" : index === 11 ? "Cierre" : current.evaluation}</span><span>{String(index + 1).padStart(2, "0")} / {String(slides.length).padStart(2, "0")}</span></div></header>
      ...
      <AnimatePresence mode="wait">
        <motion.section key={`${playbackKey}-${current.id}`} className={`slide slide--${current.kind}`} initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: -12 }} transition={{ duration: reduceMotion ? 0 : .42, ease: [0.22, 1, 0.36, 1] }}>
          {current.kind === "cover" ? <LandingHeroCover reduceMotion={reduceMotion} /> : current.kind === "innovation" ? <SlideProductInnovationIntro slide={current} reduceMotion={reduceMotion} started={started} /> : current.kind === "innovationClosing" ? <SlideProductInnovationClosing slide={current} reduceMotion={reduceMotion} /> : current.kind === "product" ? <SlideProductInterfacesShowcase slide={current} reduceMotion={reduceMotion} running={started} /> : current.kind === "gallery" ? <OperatorGallery slide={current} reduceMotion={reduceMotion} /> : current.kind === "question" ? <AudienceQuestion reduceMotion={reduceMotion} /> : current.kind === "problemPoints" && index === 3 ? <ValuePropositionSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "marketPotential" ? <MarketPotentialSlide reduceMotion={reduceMotion} running={running} /> : current.kind === "problemPoints" ? <ProblemPointsSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "formation" ? <FormationSlide reduceMotion={reduceMotion} /> : current.kind === "homefeedPanorama" ? <HomefeedPanorama reduceMotion={reduceMotion} /> : current.kind === "homefeedConnection" ? <HomefeedConnection reduceMotion={reduceMotion} /> : current.kind === "eventModes" ? <EventModes reduceMotion={reduceMotion} /> : current.kind === "transportIntegration" ? <TransportIntegration reduceMotion={reduceMotion} /> : current.kind === "touristArrival" ? <TouristArrival reduceMotion={reduceMotion} /> : current.kind === "ecosystemImpact" ? <EcosystemImpact reduceMotion={reduceMotion} /> : current.kind === "regionalPotential" ? <RegionalPotential slide={current} reduceMotion={reduceMotion} /> : current.kind === "competitiveAdvantage" ? <CompetitiveAdvantageSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "impact" ? <ImpactSlide reduceMotion={reduceMotion} running={running} /> : current.kind === "businessModel" ? <Slide10BusinessModel slide={current} reduceMotion={reduceMotion} /> : current.kind === "teamExperience" ? <TeamExperienceSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "founder" ? <FounderFinaleSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "closing" ? <ClosingSlide reduceMotion={reduceMotion} /> : <><div className="slide-copy"><span className="eyebrow">{current.eyebrow}</span><h1>{current.title}</h1>{current.statement && <p>{current.statement}</p>}{current.kind === "problem" && <motion.div className="problem-thesis" initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 1.1, duration: .7 }}><span>TeCaiGO no crea un comportamiento nuevo.</span><strong>Organiza digitalmente lo que ya ocurre todos los días.</strong></motion.div>}</div><div className="slide-visual"><SlideVisual slide={current} reduceMotion={reduceMotion} /></div></>}
        </motion.section>
      </AnimatePresence>
