...
export default function PitchDeck() {
  ...
  return (
    <main className={`deck-shell ${immersive ? "deck-shell--cover" : ""}`}>
      ...
      <header className="deck-header"><Brand compact /><div className="deck-header__meta"><span>{index === 0 ? "Apertura" : index === 1 ? "Necesidad del Mercado" : index === 2 ? "Innovación" : current.evaluation}</span><span>{String(index + 1).padStart(2, "0")} / {String(slides.length).padStart(2, "0")}</span></div></header>
      ...
      <AnimatePresence mode="wait">
        <motion.section key={`${playbackKey}-${current.id}`} className={`slide slide--${current.kind}`} initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: -12 }} transition={{ duration: reduceMotion ? 0 : .42, ease: [0.22, 1, 0.36, 1] }}>
          {current.kind === "cover" ? <LandingHeroCover reduceMotion={reduceMotion} /> : current.kind === "innovation" ? <SlideProductInnovationIntro slide={current} reduceMotion={reduceMotion} started={started} /> : current.kind === "innovationClosing" ? <SlideProductInnovationClosing slide={current} reduceMotion={reduceMotion} /> : current.kind === "product" ? <SlideProductInterfacesShowcase slide={current} reduceMotion={reduceMotion} running={started} /> : current.kind === "gallery" ? <OperatorGallery reduceMotion={reduceMotion} /> : current.kind === "question" ? <AudienceQuestion reduceMotion={reduceMotion} /> : current.kind === "problemPoints" ? <ProblemPointsSlide slide={current} reduceMotion={reduceMotion} /> : current.kind === "formation" ? <FormationSlide reduceMotion={reduceMotion} /> : current.kind === "homefeedPanorama" ? <HomefeedPanorama reduceMotion={reduceMotion} /> : current.kind === "homefeedConnection" ? <HomefeedConnection reduceMotion={reduceMotion} /> : current.kind === "eventModes" ? <EventModes reduceMotion={reduceMotion} /> : current.kind === "transportIntegration" ? <TransportIntegration reduceMotion={reduceMotion} /> : current.kind === "touristArrival" ? <TouristArrival reduceMotion={reduceMotion} /> : current.kind === "ecosystemImpact" ? <EcosystemImpact reduceMotion={reduceMotion} /> : current.kind === "regionalPotential" ? <RegionalPotential slide={current} reduceMotion={reduceMotion} /> : current.kind === "businessModel" ? <Slide10BusinessModel slide={current} reduceMotion={reduceMotion} /> : current.kind === "founder" ? <FounderFinaleSlide slide={current} reduceMotion={reduceMotion} /> : <><div className="slide-copy"><span className="eyebrow">{current.eyebrow}</span><h1>{current.title}</h1>{current.statement && <p>{current.statement}</p>}{current.kind === "problem" && <motion.div className="problem-thesis" initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : 1.1, duration: .7 }}><span>TeCaiGO no crea un comportamiento nuevo.</span><strong>Organiza digitalmente lo que ya ocurre todos los días.</strong></motion.div>}</div><div className="slide-visual"><SlideVisual slide={current} reduceMotion={reduceMotion} /></div></>}
        </motion.section>
      </AnimatePresence>
